...
//  July, 1995
//  lang@cs.cornell.edu
//  https://people.ece.cornell.edu/land/OldStudentProjects/cs490-94to95/clang/kriging.html
...
public actor Kriging  {
    ...
    func train(t: [Double], x: [Double], y: [Double], model: SWKrigingMode, sigma2: Double, alpha: Double) async {
        self.t = t
        self.x = x
        self.y = y
...
//                    }
    ...
    // Model prediction
    func predict(x: Double, y: Double) async -> Double {
        if self.error == KrigingError.none,
           let _M = self.M,
